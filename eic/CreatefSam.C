...
int CreatefSamHist(
  ...
  const std::string &ethr_path
);
std::vector<double> calculate_fSam_mean(
  const std::string &input_fname,
  const std::string &info,
  const std::string &ethr_fname
);

void CreatefSam() {
  for (int i = 0; i < files.size(); ++i) {
    path_string = std::get<0>(files[i]);
    CreatefSamHist(path_string, std::get<2>(files[i]), std::get<3>(files[i]));
  }
}
...
int CreatefSamHist(
  ...
  if (particle_name == "photon") {
    E_starts = photon_ebins;
    nEHistos = E_starts.size();
  } else {
    E_starts = electron_ebins;
    nEHistos = E_starts.size();
  }
  ...
  TFile *Tfile =
      new TFile(Form("%s/fsam_%s_%u_%u.root", path_prefix.c_str(),
  ...
  auto etaLoop = [&](int i) {
    ...
    for (size_t j = 0; j < nEtaHistos; j++) {
      ...
      const std::string ethr_fname = fmt::format("{}/sim/sim_{}.root", ethr_path, sim_info);

      auto calculate_result = calculate_fSam_mean(input_fname, sim_info, ethr_fname);
  ...
  */
  ...
  tg2d->SaveAs(fmt::format("{}/{}_{}.root", savePath, particle_name, ts.GetDate()).c_str());
...
const std::string &ethr_fname
  ...
  TFile f0(TString(input_fname), "READ");
  TFile f1(TString(ethr_fname), "READ");
  auto t0 = f0.Get<TTree>("events");
  auto t1 = f1.Get<TTree>("events");
  t0->AddFriend(t1, "ethr_tree");

  ROOT::RDataFrame d0(*t0);
  ...
  */
  ...
  auto Ethr = [](std::vector<edm4eic::ReconstructedParticleData> const
    ...
    return sum / generatedParticles.size(); // NOTE: what dividing the number of GPs does?
  };

  auto Ethr_sensitive = [](std::vector<edm4hep::SimCalorimeterHitData>& evt) {
    double total_ethr = 0.0;
  ...
  if (ethr_fname.size() != 0)
    d1 = ROOT::RDF::RNode(d0.Define("Ethr", Ethr_sensitive, {"EcalBarrelScFiHits"}));
