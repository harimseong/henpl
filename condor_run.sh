...
export TEST_INPUT_FILES=("file1" "file2" "file3" "file4" "file5")
export JOB_SIZE=${#TEST_INPUT_FILES[@]}

echo "input file list:"
for i in $(seq 0 $((JOB_SIZE - 1)))
do
    TEST_INPUT_FILES[$i]=${PREFIX_INPUT}/${TEST_INPUT_FILES[$i]}"
    echo "${TEST_INPUT_FILES[$i]}"
done

export TEST_OUTPUT_FILES=("output1" "output2" "output3" "output4" "output5")
########################################################
# executable                                           #
########################################################
export JOB_EXE=test_work.sh
